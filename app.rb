...
class App
  ...
  def create_student
    ...
    have_permission = have_permission == 'y' ? true : false
  ...
  def create_rental
    ...
    puts "People List"
      ...
      @teachers.each_with_index do |teacher, index|
        puts "#{@students.length + index + 1}) [Teacher #{index + 1}] Name: #{teacher.name}, ID: #{teacher.id}, Age: #{teacher.age}"
      end
    end
  ...
  def list_rentals_by_id
    ...
    if person_rentals.length == 0
      ...
      person_rentals.each_with_index do |rental, index|
        puts "#{index + 1}) [Rental #{index + 1}] Date: #{rental.date}, Book: #{rental.book.title} written by #{rental.book.author}"
      end
    end
  end
